...
const ACTION_URLS: readonly ActionUrlDefinition[] = [
	{
		...
		handle: () => vscode.commands.executeCommand('anthropic-copilot.setApiKey'),
		resolveFailureMessage: 'Failed to resolve Anthropic set API key URI',
	},
	{
		key: 'showLogs',
		path: SHOW_LOGS_URI_PATH,
		handle: () => logger.show(),
		resolveFailureMessage: 'Failed to resolve Anthropic show logs URI',
		setUrl: setProviderNoticeShowLogsUrl,
	},
	{
		path: SET_VISION_MODEL_URI_PATH,
		handle: () => vscode.commands.executeCommand('anthropic-copilot.setVisionModel'),
		resolveFailureMessage: 'Failed to resolve Anthropic set vision model URI',
